...
{
    // Convert Neodark:// to Neodark:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Neodark://"))
    {
        uri.replace(0, 12, "Neodark:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Neodark.lnk";
}
...
{
    return GetAutostartDir() / "Neodark.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Neodark-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Neodark-qt [" + tr("command-line options") + "]                     " + "\n";
